...
final class CM91_Git_Deployer {
    ...
    public static function routes() {
        register_rest_route(self::ROUTE_NS, '/push', [
            'methods' => 'POST',
            'callback' => [__CLASS__, 'deploy'],
    ...
    public static function status() {
        return new WP_REST_Response([
            ...
            'last_deploy' => get_option(self::OPTION_LAST, null),
        ], 200);
    }
    ...
    public static function deploy(WP_REST_Request $request) {
        ...
        if (empty($files['package']['tmp_name']) || !is_uploaded_file($files['package']['tmp_name'])) {
            return new WP_Error('cm91_missing_package', 'No deployment package received.', ['status' => 400]);
        }

        $file = $files['package'];
        if (!empty($file['size']) && (int) $file['size'] > self::MAX_BYTES) {
            return new WP_Error('cm91_too_large', 'Deployment package exceeds 50 MB.', ['status' => 413]);
        }
        ...
        if (!$signature || !hash_equals($expected, $signature)) {
            return new WP_Error('cm91_bad_signature', 'Invalid deployment signature.', ['status' => 401]);
        }
        ...
        wp_mkdir_p($stage_root);
        wp_mkdir_p($backup_root);
        $stage = $stage_root . 'deploy-' . gmdate('Ymd-His') . '-' . wp_generate_password(8, false, false) . '/';
        wp_mkdir_p($stage);

        $zip = new ZipArchive();
        if ($zip->open($tmp) !== true) {
            self::rrmdir($stage);
            return new WP_Error('cm91_bad_zip', 'Deployment package is not a valid ZIP archive.', ['status' => 400]);
        }

        for ($i = 0; $i < $zip->numFiles; $i++) {
            $name = str_replace('\\', '/', (string) $zip->getNameIndex($i));
            if ($name === '' || str_starts_with($name, '/') || preg_match('#(^|/)\.\.(/|$)#', $name) || strpos($name, "\0") !== false) {
                $zip->close(); self::rrmdir($stage);
                return new WP_Error('cm91_unsafe_zip', 'Unsafe path found in deployment package.', ['status' => 400]);
            }
            $top = strtok($name, '/');
            if (!in_array($top, ['plugins', 'themes'], true)) {
                $zip->close(); self::rrmdir($stage);
                return new WP_Error('cm91_unexpected_path', 'Only plugins/ and themes/ are allowed.', ['status' => 400]);
            }
        }

        if (!$zip->extractTo($stage)) {
            $zip->close(); self::rrmdir($stage);
            return new WP_Error('cm91_extract_failed', 'Could not extract deployment package.', ['status' => 500]);
        }
        $zip->close();

        $items = [];
        foreach ([
            'plugins' => [trailingslashit($stage . 'plugins'), trailingslashit(WP_PLUGIN_DIR)],
            'themes' => [trailingslashit($stage . 'themes'), trailingslashit(get_theme_root())],
        ] as $type => $paths) {
            ...
            foreach (glob($source_base . '*', GLOB_ONLYDIR) ?: [] as $source) {
                ...
                if (!preg_match('/^[A-Za-z0-9._-]+$/', $name)) {
                    self::rrmdir($stage);
                    return new WP_Error('cm91_bad_name', 'Invalid directory name: ' . $name, ['status' => 400]);
                }
        ...
        if (!$items) {
            self::rrmdir($stage);
            return new WP_Error('cm91_empty', 'Deployment package contained no managed directories.', ['status' => 400]);
        }

        $backup = $backup_root . preg_replace('/[^A-Za-z0-9._-]/', '-', $commit) . '-' . gmdate('Ymd-His') . '/';
        wp_mkdir_p($backup);
        $changed = [];
        ...
        try {
            foreach ($items as &$item) {
                ...
                if ($item['had_target']) {
                    wp_mkdir_p(dirname($item['backup']));
                    if (!self::move_dir($item['target'], $item['backup'])) throw new RuntimeException('Could not back up ' . $item['name']);
                }
            }
            unset($item);

            foreach ($items as $item) {
                if (!self::move_dir($item['source'], $item['target'])) throw new RuntimeException('Could not install ' . $item['name']);
                $changed[] = $item['type'] . '/' . $item['name'];
            }
        } catch (Throwable $e) {
            foreach (array_reverse($items) as $item) {
                if (is_dir($item['target'])) self::rrmdir($item['target']);
                if (!empty($item['had_target']) && is_dir($item['backup'])) self::move_dir($item['backup'], $item['target']);
            }
            self::rrmdir($stage);
            return new WP_Error('cm91_deploy_failed', $e->getMessage(), ['status' => 500]);
        }

        self::rrmdir($stage);
        self::prune_backups($backup_root, 5);
    ...
    private static function move_dir($from, $to) {
        if (!is_dir($from)) return false;
        wp_mkdir_p(dirname($to));
        if (@rename($from, $to)) return true;
        if (!self::copy_dir($from, $to)) return false;
        self::rrmdir($from);
        return true;
    }

    private static function copy_dir($from, $to) {
        if (!wp_mkdir_p($to) && !is_dir($to)) return false;
        ...
        foreach ($it as $item) {
            ...
            $dest = $to . '/' . $relative;
            if ($item->isDir()) {
                if (!wp_mkdir_p($dest) && !is_dir($dest)) return false;
            } else {
                wp_mkdir_p(dirname($dest));
                if (!@copy($item->getPathname(), $dest)) return false;
            }
        }
        return true;
    }
    ...
    private static function prune_backups($root, $keep) {
        $dirs = glob(trailingslashit($root) . '*', GLOB_ONLYDIR) ?: [];
        usort($dirs, static fn($a, $b) => filemtime($b) <=> filemtime($a));
        foreach (array_slice($dirs, $keep) as $dir) self::rrmdir($dir);
    }

    public static function admin_menu() {
        add_management_page('CM91 Git Deploy', 'CM91 Git Deploy', 'manage_options', 'cm91-git-deployer', [__CLASS__, 'admin_page']);
    }

    public static function rotate_secret() {
        if (!current_user_can('manage_options')) wp_die('Not allowed.');
        check_admin_referer('cm91_git_deployer_rotate');
        update_option(self::OPTION_SECRET, wp_generate_password(64, false, false), false);
        wp_safe_redirect(admin_url('tools.php?page=cm91-git-deployer&rotated=1'));
        exit;
    }
    ...
    public static function admin_page() {
        ...
        ?>
        <div class="wrap">
            <h1>CM91 Git Deploy</h1>
            <p>GitHub-Deployments für verwaltete Plugins und Themes auf comiker91.de.</p>
            ...
            <p><a class="button" href="<?php echo esc_url(wp_nonce_url(admin_url('admin-post.php?action=cm91_git_deployer_rotate'), 'cm91_git_deployer_rotate')); ?>" onclick="return confirm('Secret rotieren? Danach GitHub Secret aktualisieren.');">Secret rotieren</a></p>
        </div>
        <?php
    }
}
